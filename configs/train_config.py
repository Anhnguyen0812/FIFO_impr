...
DATA_LIST_RF = '/root/data/Foggy_Zurich/lists_file_names/RGB_light_filenames.txt'
DATA_DIR = '/root/data'
...
def get_arguments():
    ...
    parser.add_argument("--lambda-boundary", type=float, default=0.1, 
                        help="Weight for dual encoder (DINOv3+SAM2) boundary loss")
    parser.add_argument("--file-name", type=str, required=True)
    parser.add_argument("--modeltrain", type=str, required=True,
                        help="'no' for pretraining fogpass filter, 'train' for full FIFO training")
    
    # SegFormer specific arguments
    parser.add_argument("--use-segformer", action="store_true", default=False,
                        help="Use SegFormer MIT-B5 instead of ResNet-101")
    parser.add_argument("--segformer-model", type=str, 
                        default="nvidia/segformer-b5-finetuned-cityscapes-1024-1024",
                        help="Pretrained SegFormer model name or path")
    parser.add_argument("--freeze-segformer-encoder", action="store_true", default=False,
                        help="Freeze SegFormer encoder during training (only train decoder)")
    parser.add_argument("--accum-steps", type=int, default=1,
                        help="Gradient accumulation steps (useful for large models like SegFormer)")
    
    # FogPassFilter preprocessing compatibility
    parser.add_argument("--retrain-fogpass", action="store_true", default=False,
                        help="Retrain FogPassFilter from scratch (ignore pretrained weights). "
                             "Use when switching between RGB/BGR preprocessing.")
    parser.add_argument("--adapt-fogpass", action="store_true", default=False,
                        help="Adapt pretrained FogPassFilter to new preprocessing with lower LR. "
                             "Use when switching between RGB/BGR preprocessing.")
